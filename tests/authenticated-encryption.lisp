...
(let ((secret (ironclad:ascii-string-to-byte-array "01234567890abcde"))
  ...
  (is (authenticated-encrypt plain1 :secret secret :nonce nonce)
      enc1
      :test #'equalp)
  (is (authenticated-decrypt enc1 :secret secret)
      plain1
      :test #'equalp)
  (is (authenticated-encrypt plain2 :secret secret :nonce nonce)
      enc2
      :test #'equalp)
  (is (authenticated-decrypt enc2 :secret secret)
      plain2
      :test #'equalp))
